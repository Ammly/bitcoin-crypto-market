      ...
      <div className="overflow-x-auto overflow-y-visible pb-4">
        <div className="inline-block min-w-full">
          <div 
            ...
            style={{ 
              gridTemplateColumns: `120px repeat(${symbols.length}, 65px)`,
          ...
          >
            ...
            {symbols.map(symbol => (
              <div 
                key={symbol} 
                className="text-xs font-semibold text-gray-900 text-center py-2 px-1 bg-white"
              >
                <div className="truncate" title={symbol}>
                  {symbol}
                </div>
              </div>
            ))}
            ...
            {matrix.map((row, i) => (
              <React.Fragment key={`row-${i}`}>
                <div className="sticky left-0 z-10 bg-white text-xs font-semibold text-gray-900 py-2 pr-3 text-right flex items-center justify-end border-r-2 border-gray-100">
                  <span className="truncate" title={row.symbol as string}>
                    {row.symbol}
                  </span>
                </div>
                {symbols.map((symbol, j) => {
                  ...
                  return (
                    <div
                      key={`${i}-${j}`}
                      className={`${getColor(value)} flex items-center justify-center text-xs font-bold h-16 rounded shadow-sm transition-transform hover:scale-105 hover:z-20 cursor-pointer`}
